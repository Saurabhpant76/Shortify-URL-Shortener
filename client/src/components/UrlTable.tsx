            ...
            <tbody>
              {loading
                ...
                : urls.map((url) => {
                    ...
                    // TypeScript ko bypass karne ke liye (in case index.ts update nahi hui hai)
                    const urlTags = (url as any).tags as string[] | undefined;

                    return (
                      <tr key={url._id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                        
                        {/* 1. Yahan humne Original URL aur Tags Badges ko ek sath rakha hai */}
                        <td className="px-4 py-3 max-w-xs">
                          <a
                            href={url.originalUrl}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="text-gray-300 hover:text-brand-400 transition-colors block truncate"
                            title={url.originalUrl}
                          >
                            {truncateUrl(url.originalUrl, 45)}
                          </a>
                          
                          {/* 2. Tags Badges Render Logic */}
                          {urlTags && urlTags.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-1.5">
                              {urlTags.map((tag, idx) => (
                                <span 
                                  key={idx} 
                                  className="px-2 py-0.5 rounded text-[10px] font-semibold bg-brand-900/30 text-brand-300 border border-brand-800/50 uppercase tracking-wider shadow-sm"
                                >
                                  {tag}
                                </span>
                              ))}
